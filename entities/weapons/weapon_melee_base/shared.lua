			...
			if SERVER then
				if ent:IsPlayer() then
					...
					if ent:GetAimVector():DotProduct(self.Owner:GetAimVector()) > 0  then
						...
						local multiplier = hook.Call("OnPlayerBackstab", GAMEMODE, self.Owner, self, ent) or 2
						dmgInfo:SetDamage(self.Damage * multiplier)
						ent:ViewPunch(Angle(15, 0, 0))
					else
						local ref = ent:GetActiveWeapon()
						
						if ref then	
							if ref.Blocking then
...
if SERVER then
	util.AddNetworkString("SendBlocking")
	util.AddNetworkString("SendAnim")
	
	net.Receive("SendBlocking",function()
...
if CLIENT then
	net.Receive("SendAnim", function()
		net.ReadEntity():SetHoldType(net.ReadString())
	end)
end
...
function SWEP:Think()
	...
	if self.Owner:KeyDown(IN_ATTACK2) && !self.Owner:KeyPressed(IN_ATTACK) && !self.Owner:IsSprinting() then	
		if ( !self.BlockVec ) then return end
		if self.Blocking == true then return end
